...
export default function VideoPlayer({
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative rounded-2xl overflow-hidden shadow-2xl border border-border-subtle bg-border/20 group ${className}`}
    ...
    >
      <div className="aspect-video bg-linear-to-br from-border/40 to-border/20 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-bg/60 via-transparent to-transparent z-20"></div>
        ...
        {showControls && (!isPlaying || isHovered) && (
          <div className="absolute inset-0 flex items-center justify-center z-25 transition-opacity duration-300">
            <button
              onClick={togglePlay}
              className="w-16 h-16 bg-accent/30 rounded-full flex items-center justify-center hover:bg-accent/40 transition-all duration-300 scale-110 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-bg backdrop-blur-sm"
